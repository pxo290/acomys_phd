...
#SBATCH -o /ei/projects/b/b881ad24-2c3c-4508-a6b2-b1c552453c24/slurm_outputs/2023-05-18_V2_prep_for_Cactus_all_assemblies_strict_bins_selected_references-%A.out
#SBATCH -e /ei/projects/b/b881ad24-2c3c-4508-a6b2-b1c552453c24/slurm_outputs/2023-05-18_V2_prep_for_Cactus_all_assemblies_strict_bins_selected_references-%A.err
...
#This script can be found on the hpc at: /hpc-home/osbourne/Post-EI_scripts/2023-05-18_V2_prep_for_Cactus_all_assemblies_strict_bins_selected_references.sh
...
export outdir="${inpdir}/softmasked_2"
export list_of_files_file="${inpdir}/list_files_for_cactus.txt"
...
execute_preprocess () {
    input_thing="$1"
    input_file=$(find -maxdepth 1 -type f -name "$input_thing")
    if [[ "$input_file" == "./.fa" ]]
    then
        ...
        output_dir="$2"
        #First will do the contig level filtering, removing any under 500bp in length
        seqkit seq -m 500 --quiet "$input_file" > "${output_dir}/${input_name}.temp1.fa"
        #Now the softmasking since Cactus can't handle the 
        ...
        sed -i -e 's@/@ @g' "${output_dir}/${input_name}.temp2.fa"
        sed -i -e 's@\\@ @g' "${output_dir}/${input_name}.temp2.fa"
...
#Finding target fasta files, presumes that you have manually copied them into the directory
while IFS= read -r line
do
  subject_file="$line"
  ...
  then
    ...
    execute_preprocess "$subject_file" "$outdir"
  fi 
...
#Running JolyTree
srun "$jolyTree_script" -i "$outdir" -s 0.99 -c 0.25 -a 1.5 -b "$newick_name_marker" -t 1 -r 500
