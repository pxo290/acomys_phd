...
#SBATCH -J 2023-06-24_Sibeliaz_isolates_and_15_references
#SBATCH -c 1
#SBATCH --mem=612gb
#SBATCH --mail-type=begin,end,fail
#SBATCH --mail-user=peter.osborne@earlham.ac.uk
#SBATCH -o /ei/projects/b/b881ad24-2c3c-4508-a6b2-b1c552453c24/slurm_outputs/2023-06-24_Sibeliaz_isolates_and_15_references-%A.out
#SBATCH -e /ei/projects/b/b881ad24-2c3c-4508-a6b2-b1c552453c24/slurm_outputs/2023-06-24_Sibeliaz_isolates_and_15_references-%A.err

#Written on 2023-06-24
#Written by Peter Osborne
#Written to be run on the NBI HPC

#This script will run SibeliaZ on softmasked isolate assemblies and 15 randomly selected references. Note that it no longer has a manually provided time limit so will be the default of the ei-medium queue.

#This script can be found on the hpc at: /hpc-home/osbourne/Post-EI_scripts/2023-06-24_Sibeliaz_isolates_trial.sh

#Tweaked failure catch function
failure_catch() {
  local lineno=$1
  local msg=$2
  printf "Job was a run of script:\t/hpc-home/osbourne/Post-EI_scripts/2023-06-24_Sibeliaz_isolates_trial.sh\nIt started around:\t${script_start_time}\n"
  printf "\nFailed at:\t$lineno\t$msg\n"
}
trap 'failure_catch ${LINENO} "$BASH_COMMAND"' ERR

#Setting fixed variables
export isolates_dir="/ei/projects/b/b881ad24-2c3c-4508-a6b2-b1c552453c24/culture_isolates/assemblies/all_contig_assemblies_contigs_under_500_bp_removed/assemblies_bins_iMGMC_MAGs_and_some_references/isolate_assemblies_limited_references_for_SibeliaZ"
...
export target_assemblies=$(find -maxdepth 1 -type f -name "*.fa" -printf '%f\n')
for f in $target_assemblies
do
  assembly_name=$(basename "$f" | awk -F ".f" '{print $1}')
  printf ">$assembly_name\n" > "${output_dir}/${assembly_name}.isolates_only.Single_contig.Softmasked.fa"
  grep -v ">" "$f" | tr -d "\n" | sed 's/$/\n/g' >> "${output_dir}/${assembly_name}.isolates_only.Single_contig.Softmasked.fa"
done
...
cat *.isolates_only.Single_contig.Softmasked.fa > "SibeliaZ.all_softmasked_isolates.Multifasta_for_alignment.fa"

#Removing the files not needed any more
rm *.isolates_only.Single_contig.Softmasked.fa

#Finding all the files and putting them into a variable which is their basenames separated by spaces for SibeliaZ
export inpf="${output_dir}/SibeliaZ.all_softmasked_isolates.Multifasta_for_alignment.fa"
