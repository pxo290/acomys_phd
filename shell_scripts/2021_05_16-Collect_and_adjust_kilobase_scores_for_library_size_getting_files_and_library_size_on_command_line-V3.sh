...
#SBATCH -p ei-medium
#SBATCH -c 2
#SBATCH -J V3TaxFromContigs&Adjust
#SBATCH --mem=16gb
...
do
  ...
  do
    ...
    contigs=$(grep "£${taxon}@" 2021_05_16-All_taxons_and_their_contigs_at_sign_between_taxa_and_contigs_commas_between_contigs.txt.Taxons_all_start_with_pound_sign.txt | awk -F "@" '{print $2}' | tr "," "\n" | sort -V | tr "\n" "," | sed 's/,*$//g') || echo "Failed line 37"
  ...
  done < 2021_05_16_all_unique_taxa_punctuation_removed.txt || echo "Failed line 55"
  printf "\nProcessed file:\t$f\n"
done
mv 
