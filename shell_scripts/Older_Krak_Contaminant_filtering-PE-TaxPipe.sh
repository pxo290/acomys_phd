...
printf "\nThe host genome provided is: $host_genome\n"
printf "\nThe human genome provided is: $human_genome\n"

printf "\n !!!!! Note, this script uses the older of the two Kraken2 databases found at: /ei/public/databases/kraken/2.0.7_std !!!!!\n"
...
#Consider making this bit into a function#
...
bbduk_run () {
  ...
  export mdn=$7
  wkdir=$8
  printf "Running read files $rf1 and $rf2 through BBDUK to remove host and human mapping reads."
  printf "Host genome which will be used is: $hog"
  printf "Human genome which will be used is: $hug"
  printf "Emails will be sent to user-provided address at: $uem"
  printf "Output files will be named with this: $cid"
  printf "Output files will also end their names with this: $mdn"
  srun bbduk.sh -Xmx120g -Xms55g -prealloc in=$rf1 in2=$rf2 ref=$hog,$hug outm=${cid}_host_or_human_${mdn} out=${cid}_cleansed_${rf1} out2=${cid}_cleansed_${rf2} overwrite=t stats=${mdn}_host_comparison_stats.txt
  printf "\n The read count reads matching host or human from sample $mdn is: " >> $works_dir/${cid}_${mdn}_contaminant_read_counts.txt && grep -c "@" ${cid}_host_or_human_${mdn} >> $works_dir/${cid}_${mdn}_contaminant_read_counts.txt
}
bbduk_run $RFQ1 $RFQ2 $host_genome $human_genome $user_input_mail $CID $MDN $works_dir
...
export RFQ1=${cid}_cleansed_${rf1}
export RFQ2=${cid}_cleansed_${rf2}
...
printf "\nDefining function for Kraken2 run with different confidence scores using older database.\n"
old_kraken2_run () {
  R1=$1
  R2=$2
  CS=$3
  ...
  kraken2 --paired --db /ei/public/databases/kraken/2.0.7_std --threads 10 --confidence $CS --unclassified-out ${mdn}_OLDER_Kraken2-Unclassified_confidence_score${CS}_#.txt --classified-out ${mdn}_OLDER_Kraken2-classified_confidence_score${CS}_#.txt $R1 $R2 --report ${mdn}_OLDER_Kraken2_confidence_score_${CS}_report.out
  mv ${mdn}_OLDER_Kraken2_confidence_score_${CS}_report.out ${mdn}_OLDER_Kraken2_confidence_score_${CS}_report.txt
  mail -s "Progress Monitoring 85763000" -a ${mdn}_OLDER_Kraken2_confidence_score_${CS}_report.txt "$user_input_mail" <<< "Kraken2 report for read files $R1 and $R2 with confidence score $CS is attached"
  mv ${mdn}_OLDER_Kraken2_confidence_score_${CS}_report.txt $works_dir/Kraken2
  mv ${mdn}_OLDER_Kraken2-classified_confidence_score${CS}_* $works_dir/Kraken2
  mv ${mdn}_OLDER_Kraken2-Unclassified_confidence_score${CS}_* $works_dir/Kraken2
}
printf "\nRunning Kraken2 with different confidence scores \n"
old_kraken2_run $RFQ1 $RFQ2 1.00 $MDN
old_kraken2_run $RFQ1 $RFQ2 0.95 $MDN
old_kraken2_run $RFQ1 $RFQ2 0.75 $MDN
old_kraken2_run $RFQ1 $RFQ2 0.50 $MDN
old_kraken2_run $RFQ1 $RFQ2 0.25 $MDN
old_kraken2_run $RFQ1 $RFQ2 0.10 $MDN
old_kraken2_run $RFQ1 $RFQ2 0.05 $MDN
...
export krakproc=$(find -maxdepth 1 -type f -name "*-and_unclassified_sorted_OLDER_DATABASE.txt")
...
# scp ${mdn}_OLDER_Kraken2-classified_confidence_score${CS}_* $DPL
# scp ${mdn}_OLDER_Kraken2-Unclassified_confidence_score${CS}_* $DPL
cd $works_dir/Metaphlan2/
for f in $metaphlan_reports
do
  printf "\n Copying $f to the departures lounge, $DPL"
