...
do
  start_time=$(date)
  echo "Starting diamond run at: $start_time"
  ...
  diamond blastx -p 20 -d /ei/scratch/osbourne/RETRO-DIAMOND/nr.dmnd -q $f -o ${f%}-diamond-output.daa -f 100
  end_time=$(date) && echo "Finishing diamond run at: $end_time" && mail -s "Progress Monitoring 85763000" peter.osborne@earlham.ac.uk <<< "\-\-\ DIAMOND UPDATE /-/-/ Ran diamond 0.9.10 on '$f' finishing at $end_time"
done
