...
#SBATCH -c 4
#SBATCH -J 2022-06-24_RS_trial_workflow_capybara_version
#SBATCH --mem=825gb
...
#Modified from original on 2022-07-06
#Testing it out with some Capybara metagenomic reads, from https://www.ebi.ac.uk/ena/browser/view/SAMN12698194?show=reads
#they are part of Study MGYS00005844 (MGnify)/Project: PRJNA563062 (ENA)
...
sample_nome="${raw_inpf1%.*}"
base_sample_name=$(echo "$sample_nome" | awk -F "_" '{print $1}')
...
execute_QC_and_cleanse () {
  ...
  cp "$func_inpf1" "${outdir}/Capybara_${sampleID}_R1.fastq"
  cp "$func_inpf2" "${outdir}/Capybara_${sampleID}_R2.fastq"
  #First up will do QC using FastP
  cd "$outdir"
  fastp --in1 "${outdir}/Capybara_${sampleID}_R1.fastq" --in2 "${outdir}/Capybara_${sampleID}_R2.fastq" --out1 "${outdir}/Capybara_${sampleID}_R1.QCed.fastq" --out2 "${outdir}/Capybara_${sampleID}_R2.QCed.fastq" --json "${outdir}/Capybara_${sampleID}.QCed.json" --html "${outdir}/Capybara_${sampleID}.QCed.html" --verbose --report_title "Capybara_${sampleID}-report"
  #Setting reference genome variables for the host, humans and other possible eukaryotes
  capybara_genome="/ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/extra_ref_genomes/Capybara_Genbank_GCA_004027455.1_HydHyd_v1_BIUU_genomic.fna"
  ...
  wild_strawberry_genome="/ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/extra_ref_genomes/Wild_strawberry_GCF_000184155.1_FraVesHawaii_1.0_genomic.fna"
  ...
  srun bbduk.sh -Xmx650g -Xms64g -prealloc in="Capybara_${sampleID}_R1.QCed.fastq" in2="Capybara_${sampleID}_R2.QCed.fastq" ref="$capybara_genome","$human_genome","$mus_musculus_genome","$euro_rabbit_genome","$rock_pigeon_genome","$common_shrew_genome","$rattus_rattus_genome","$chicken_genome","$euro_frog_genome","$wild_strawberry_genome" outm="${outdir}/Capybara_${sampleID}.QCed.Host_or_human_matching.fastq" out="Capybara_${sampleID}_R1.QCed.Cleansed.fastq" out2="Capybara_${sampleID}_R2.QCed.Cleansed.fastq" overwrite=t stats="${outdir}/Capybara_${sampleID}-BBDuk_stats.txt"
  ...
  printf "File\tRaw read count\tQC'ed read count\tCleansed read count\n" > "${outdir}/Capybara_${sampleID}_read_numbers_different_files.tsv"
  raw_inpf1_read_count=$(grep -c "@A0" "${outdir}/Capybara_${sampleID}_R1.fastq")
  QCed_inpf1_read_count=$(grep -c "@A0" "${outdir}/Capybara_${sampleID}_R1.QCed.fastq")
  cleansed_inpf1_read_count=$(grep -c "@A0" "${outdir}/Capybara_${sampleID}_R1.QCed.Cleansed.fastq")
  raw_inpf2_read_count=$(grep -c "@A0" "${outdir}/Capybara_${sampleID}_R2.fastq")
  QCed_inpf2_read_count=$(grep -c "@A0" "${outdir}/Capybara_${sampleID}_R2.QCed.fastq")
  cleansed_inpf2_read_count=$(grep -c "@A0" "${outdir}/Capybara_${sampleID}_R2.QCed.Cleansed.fastq")
  ...
  rm "${outdir}/Capybara_${sampleID}_R1.fastq"
  rm "${outdir}/Capybara_${sampleID}_R2.fastq"
  pigz -9 -p 4 "${outdir}/Capybara_${sampleID}_R1.QCed.fastq"
  pigz -9 -p 4 "${outdir}/Capybara_${sampleID}_R2.QCed.fastq"
  #Also really need to zip up the host/eukaryote matching reads fastq since it can
  #get really, really big
  pigz -9 -p 4 "${outdir}/Capybara_${sampleID}.QCed.Host_or_human_matching.fastq"
  #Noting that the function finished
  printf "\nCompleted a QC and cleanse function execution on input files:\t$func_inpf1 and $func_inpf2"
  printf "\nUsed the following genomes for cleansing:\t$euro_Capybara_genome, $human_genome ,$mus_musculus_genome ,$euro_rabbit_genome ,$rock_pigeon_genome ,$common_shrew_genome ,$rattus_rattus_genome ,$chicken_genome and $common_wheat_genome\n"
}
...
execute_QC_and_cleanse "$raw_inpf1" "$raw_inpf2" "$base_sample_name"
...
qced_cleansed_inpf1="${qced_cleansed_inpdir}/Capybara_${base_sample_name}_R1.QCed.Cleansed.fastq"
qced_cleansed_inpf2="${qced_cleansed_inpdir}/Capybara_${base_sample_name}_R2.QCed.Cleansed.fastq"
sample_ID="$base_sample_name"
