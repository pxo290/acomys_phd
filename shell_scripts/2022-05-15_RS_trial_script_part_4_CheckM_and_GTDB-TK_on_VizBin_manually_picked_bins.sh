...
#SBATCH -c 4
#SBATCH -J 2022-05-15_RS_trial_script_part_4_CheckM_and_GTDB-TK_on_MaxBin2_bins
#SBATCH --mem=725gb
#SBATCH --mail-type=begin,end,fail
#SBATCH --mail-user=peter.osborne@earlham.ac.uk
#SBATCH -o /ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/slurm_outputs/2022-05-15_RS_trial_script_part_4_CheckM_and_GTDB-TK_on_MaxBin2_bins-%A.out
#SBATCH -e /ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/slurm_outputs/2022-05-15_RS_trial_script_part_4_CheckM_and_GTDB-TK_on_MaxBin2_bins-%A.err
...
execute_metaSPADES () {
  ...
  assembly_inpf2="$2"
  assembly_label="$3"
  assembly_output_dir="${4}/${assembly_label}_metaSPADES_assembly"
...
docker_2_image_path="${maxbin2_directory}/2022-05-25_docker_maxbin2_image_by_pegi3s.img"

#Making an output directory for the MaxBin2 output
mkdir -p "$maxbin2_directory/${sample_ID}_MaxBin2_output"
...
mkdir -p "$maxbin2_directory/${sample_ID}_MaxBin2_tempdir"

#Temporary TESTING VARIABLE
fetch_assembly_single_line_min_contig_size="/ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/Assembly_output/Polecat_R0786-S0015_VWT837_A64353_metaSPADES_assembly/Polecat_R0786-S0015_VWT837_A64353_metaSPADES_assembly.Contigs_min_length_500bp.single_line.fa"
###
...
execute_maxbin2_docker_version () {
  ...
  singularity exec "$maxbin2_docker_image" run_MaxBin.pl -contig "$contig_file" -reads "$read_file_1" -reads2 "$read_file_2" -min_contig_length 1000 -out "${output_directory}/${sampleID}.MaxBin2_output.out"
  ...
  #I don't know what the output of MaxBin2 is, so check what the actual contents
  #of the output file are and you can change it to a .fa if that is what it puts
  #out
}
...
execute_maxbin2_docker_version "$docker_2_image_path" "$bin_contig_file" "$bin_inpf1" "$bin_inpf2" "$bin_work_dir" "$sample_ID" "$maxbin2_directory/${sample_ID}_MaxBin2_output"
...
#   #use the -x flag to give the .fa extension since you gave the VizBin clusters
#   #that extension when you picked and saved them locally
#   #Running the CheckM taxonomy step first, to produce the files needed for the QA
#   #step
#   printf "\nStarting the CheckM lineage_wf execution on all VizBin manually selected .fa bin files from directory:\t$checkM_inpdir"
#   checkm lineage_wf -t 2 -x fa "$checkM_inpdir" "$checkM_outdir"
...
#   checkm qa -o 2 -f "${checkM_outdir}/${sample_ID}_CheckM_QA_check_all_VizBin_manually_selected_clusters_output.out" "$target_lineage_file" "$qa_output_file_dir"
#   printf "\nCompleted CheckM execution on all VizBin manually selected .fa bin files from directory:\t$checkM_inpdir"
# }
...
#   printf "\nStarting a classify_wf workflow with GTDB-TK on all VizBin manually selected .fa bin files from directory:\t$GTDBTK_inpdir"
#   gtdbtk classify_wf --genome_dir "$GTDBTK_inpdir" --out_dir "${GTDBTK_outdir}/GTDB-TK_classify_wf_output" --cpus 4 -x fa --min_perc_aa 10 --min_af 0.65 --tmpdir "$top_tempdir" --scratch_dir "$top_tempdir"
#   printf "\nFinished a classify_wf workflow with GTDB-TK on all VizBin manually selected .fa bin files from directory:\t$GTDBTK_inpdir"
# }
...
# execute_checkM "$vizbin_bins_dir" "$sample_ID" "$bin_checks_outdir"
# execute_GTDBTK "$vizbin_bins_dir" "$sample_ID" "$bin_checks_outdir" "$inpdir"
