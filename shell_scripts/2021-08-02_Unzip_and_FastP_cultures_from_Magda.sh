...
#Fixed variables
export ORIGIN=$(pwd)
export DESTINATION="/ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/Culture_reads/FastP_output/Magda_prodvided_cultures"
...
do
  ...
  export inpf2=$(echo "$inpf1" | sed 's/_R1_/_R2_/g')
  export uzinpf1=$(echo "$inpf1" | awk -F ".gz" '{print $1}' | awk 'NF')
  export uzinpf2=$(echo "$inpf2" | awk -F ".gz" '{print $1}' | awk 'NF')
  lobel=$(basename "$inpf1" | awk -F "-" '{print $3}' | awk -F "_" '{print $2}')
  export label=$(printf "${lobel}_Fastp")
  gunzip "$inpf1"
  gunzip "$inpf2"
