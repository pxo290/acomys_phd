...
#SBATCH -c 4
#SBATCH -J 2022-05-15_RS_trial_script_part_3_Assembly_and_binning
#SBATCH --mem=625gb
#SBATCH --mail-type=begin,end,fail
#SBATCH --mail-user=peter.osborne@earlham.ac.uk
#SBATCH -o /ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/slurm_outputs/2022-05-15_RS_trial_script_part_3_Assembly_and_binning-%A.out
#SBATCH -e /ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/slurm_outputs/2022-05-15_RS_trial_script_part_3_Assembly_and_binning-%A.err
...
#Part 3 of the script will use Megahit to produce assemblies from the reads left
#over after QC and host/human cleansing, then you will need to manually download
#the assembly and use VizBin to select clusters of sequences manually. MEGAHIT
#did in fact produce contigs with lengths under 1kb so make sure when using
#VizBin yourself to select only showing contigs at least 1kb in length.

#Sourcing the software
source package /tgac/software/testing/bin/megahit-1.1.1_dw #MEGAHIT v1.1.1
...
cd "Assembly_output"
assembly_outdir=$(pwd)
cd "$qced_cleansed_inpdir"

#Writing the Megahit function
execute_megahit () {
  assembly_inpf1="$1"
  assembly_inpf2="$2"
  assembly_label="$3"
  assembly_output_dir="${4}/${assembly_label}_MEGAHIT_assembly"
  min_contig_length="500" #This means that any contigs below this size in base
  #pairs will not be output by MEGAHIT
  printf "Will use MEGAHIT v1.1.1 to produce an assembly from processed PE read files:\t$assembly_inpf1 and $assembly_inpf2\n"
  #Note that this uses the assembly output directory as the temporary directory as well
  megahit -1 "$assembly_inpf1" -2 "$assembly_inpf2" --memory 0.25 --k-max 255 --k-min 99 --k-step 6 --verbose -o "$assembly_output_dir" --out-prefix "$assembly_label" --min-contig-len "$min_contig_length" --no-mercy --memory 0.99 --num-cpu-threads 4 --tmp-dir "$assembly_output_dir" --verbose
  printf "Finished the MEGAHIT assembly run to produce an assembly from processed PE read files:\t$assembly_inpf1 and $assembly_inpf2\n"
}

#Exporting the Megahit function
export -f execute_megahit

#Running the Megahit function
execute_megahit "$qced_cleansed_inpf1" "$qced_cleansed_inpf2" "$sample_ID" "$assembly_outdir"

#Now work on the assumption that you have done the manual processing of the assembly
#with VizBin locally and then re-uploaded the sequence files and images back to
#the cluster and put them in the specific directory indicated below.
vizbin_bins_dir="/ei/projects/c/c33dfa31-8d66-4727-827b-ce9eb62d3237/scratch/2022-04-05_Extra_or_side_stuff/RS_experimenting/VizBin_manually_selected_sequence_files"

#Also assumes that the files have a specific naming convention which includes at
#least the following pattern: '*_RS_Polecat_*_Megahit_min_contig_size_1kb_VizBin_manually_picked_cluster_*.fa'
#Be aware that the image files have names which end: .fa.png
