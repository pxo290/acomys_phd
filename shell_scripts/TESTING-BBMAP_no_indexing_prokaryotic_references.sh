...
bbmap_run () {
  ...
  mkdir $locale/${reference_label}
  cd $locale/${reference_label}
  destination=$(pwd)
  cd ..
  bbmap.sh ref="$reference" in=$RFQ1 in2=$RFQ2 k=15 slow=t threads=4 nodisk=t outm=$locale/${reference_label}/${lab}_matching_${reference_label}_reads statsfile=$locale/${reference_label}/${lab}__${reference_label}_stats.txt -Xmx605g -Xms435g
...
touch ${metagenome_label}_combined_results.txt
touch ${metagenome_label}_combined_matching_percentage_baseline.txt
for f in $subj_files
do
  TV1=$f
  TV2=$(basename $TV1)
...
export results_files=$(find -maxdepth 3 -type f -name "*_stats.txt")
for f in $results_files
do
  printf "\n File is: $f \n" >> ${metagenome_label}_combined_results.txt
  cat $f >> ${metagenome_label}_combined_results.txt
  printf "\n File is: $f \n" >> ${metagenome_label}_combined_matching_percentage_baseline.txt
  grep "mated pairs" $f >> ${metagenome_label}_combined_matching_percentage_baseline.txt
done
