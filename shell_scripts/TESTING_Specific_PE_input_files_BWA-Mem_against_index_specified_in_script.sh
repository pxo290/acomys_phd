...
holdvar1=$1 && gunzip "$holdvar1" && export INPF1=$(basename $holdvar1 | awk -F "." '{print $1}' | sed 's@/@@g') && printf "\nINPF1 is:\t$INPF1\n" || export INPF1=$(basename $holdvar1) && printf "\nINPF1 was already unzipped, it is:\t$INPF1\n"
holdvar2=$2 && gunzip "$holdvar2" && export INPF2=$(basename $holdvar2 | awk -F "." '{print $1}' | sed 's@/@@g') && printf "\nINPF2 is:\t$INPF2\n" || export INPF2=$(basename $holdvar2) && printf "\nINPF2 was already unzipped, it is:\t$INPF2\n"
  ...
  then
    ...
    then
      ...
      then
        ...
        samtools view -@2 -b -F 4 "${label}_BWA_against_bins.bam" > "${label}_BWA_against_bins_Mapped.bam" || printf "\nFailed trying to make a mapped reads BAM from ${label}_BWA_against_bins.bam\n"
        ...
        then
          ...
          sleep 2m
          printf "\nWill try and tidy up by moving ${label}_BWA_against_bins_Mapped.bam to $outdir, then zipping ${label}_BWA_against_bins.bam and ${label}_BWA_against_bins.sam then moving them to $outdir\n"
          mv ${label}_BWA_against_bins_Mapped.bam "$outdir" || printf "\nFailed line 85, moving ${label}_BWA_against_bins_Mapped.bam to $outdir\n"
          gzip ${label}_BWA_against_bins.bam || printf "\nFailed line 86, zipping file ${label}_BWA_against_bins.bam\n"
          gzip ${label}_BWA_against_bins.sam || printf "\nFailed line 87, zipping file ${label}_BWA_against_bins.sam\n"
          mv ${label}_BWA_against_bins.bam.gz "$outdir" || printf "\nFailed line 88, moving file ${label}_BWA_against_bins.bam.gz to $outdir\n"
          mv ${label}_BWA_against_bins.sam.gz "$outdir" || printf "\nFailed line 89, moving file ${label}_BWA_against_bins.sam.gz to $outdir\n"
